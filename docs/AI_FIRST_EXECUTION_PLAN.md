...
- При конфликте действуют `GEO_NARRATIVE_PRODUCT_STRATEGY.md`, затем `R02_NARRATIVE_PROOF.md` для решений о контенте/процессе R02, затем `GEO_NARRATIVE_TECHNICAL_SPEC.md`, затем этот план. Исторические документы не переопределяют geo-narrative решение.
...
| 2 | R02 | Story-first graph + одномиссионный Wizard-of-Oz package | R01 | готовый geo component proof |
...
## R02 — story-first graph и одномиссионный Wizard-of-Oz package
...
Authoring graph до route binding должен иметь:
- один постоянный эмоционально взаимный Напарник, который в M2 помогает игроку в ответ;
- не более пяти объявленных state variables и не более восьми путей мини-арки;
- минимум один выбор M1, который читается в M2;
- минимум две независимые clues до центрального reveal;
- reveal, который переосмысляет прежние действия и создаёт осознанный выбор;
...
- трёхмиссионный graph валидируется: нет dangling/unreachable nodes, cycles, dead ends, undeclared state и branch explosion;
- полностью произведена только M1; M2–M3 существуют как graph outline для payoff и причины вернуться;
